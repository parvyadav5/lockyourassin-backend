...
// In-memory cache for anti-repeat (reset on restart is fine for this)
const lastUsedIndex = {};
...
function getMotivationalMessage(gender, userId) {
    ...
    let index;
    // Try to find a different message than last time
    let attempts = 0;
    do {
        index = Math.floor(Math.random() * messages.length);
        attempts++;
    } while (index === lastUsedIndex[key] && messages.length > 1 && attempts < 5);
